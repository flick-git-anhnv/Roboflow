    ...
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowCreate((v) => !v)}>
          {showCreate ? 'Đóng' : '+ Thêm tài khoản'}
        </button>
      </div>

      {error && (
        <div style={{
          background: '#fff3f0', border: '1px solid #F05922', color: '#c0392b',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <label style={{ fontSize: 13 }}>
              ...
              <input type="text" value={newUsername} onChange={(e) => setNewUsername(e.target.value)}
                ...
                style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 10px', borderRadius: 6, border: '1px solid var(--border)', boxSizing: 'border-box' }} />
            </label>
            <label style={{ fontSize: 13 }}>
            ...
            <label style={{ fontSize: 13 }}>
              ...
              <select value={newRole} onChange={(e) => setNewRole(e.target.value as UserRole)}
                style={{ display: 'block', width: '100%', marginTop: 4, padding: '8px 10px', borderRadius: 6, border: '1px solid var(--border)' }}>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--navy)', color: 'var(--white)', textAlign: 'left' }}>
              ...
              {users.map((u) => (
                <tr key={u.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  <td style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {u.username}
                    {u.id === currentUser.id && <span style={{ color: '#888', fontSize: 11 }}>(bạn)</span>}
                  </td>
                  <td style={{ padding: '10px 12px' }}>{u.display_name}</td>
                  <td style={{ padding: '10px 12px' }}>
                    <select value={u.role} onChange={(e) => handleRoleChange(u, e.target.value)}
                      disabled={u.id === currentUser.id}
                      style={{ padding: '4px 6px', borderRadius: 5, border: '1px solid var(--border)' }}>
